...
exports.modStick = (media, client, kal, from) => {
    out = getRandom('.webp')
    try {
        console.log(media)
        spawn('webpmux', ['-set','exif', './lib/data.exif', media, '-o', out])
        .on('exit', () => {
            client.sendMessage(from, fs.readFileSync(out), 'stickerMessage', {quoted: kal})
            fs.unlinkSync(out)
            fs.unlinkSync(media)
        })
    } catch (e) {
        console.log(e)
        client.sendMessage(from, 'Terjadi keslahan', 'conversation', { quoted: kal })
        fs.unlinkSync(media)
    }
}
